...
export async function previewDiagnosticReportPdf(input: {
  ...
  const drawFooter = () => {
    ...
    if (footerText){
      doc.setFontSize(9)
      doc.text(footerText, 297.5, baseY - 14, { align: 'center' })
    }
  }

  const columnStyles: Record<number, any> = {}
  const idxPrev = hasPrev ? headRow.indexOf('Previous') : -1
  const idxFlag = hasFlag ? headRow.indexOf('Flag') : -1
