...
export async function printColonoscopyReport(input: {
  ...
  const normalizeValue = (v: any): string => {
    ...
    if (typeof v === 'string') {
      const t = v.trim()
      if (!t) return ''
      // If it's JSON, convert object to labeled sections
      if ((t.startsWith('{') && t.endsWith('}')) || (t.startsWith('[') && t.endsWith(']'))){
        try {
          const parsed: any = JSON.parse(t)
          if (parsed && typeof parsed === 'object' && !Array.isArray(parsed)){
            const order = ['Referred By','Indications','Consent','Procedure','Pre-Medication','Rectal Exam','Findings','Impression','Recommendations']
            const parts: string[] = []
            for (const k of order){
              const val = (parsed as any)[k] ?? (parsed as any)[k.replace(/\s+/g,'')] ?? (parsed as any)[k.toLowerCase().replace(/\s+/g,'')] 
  ...
  const bodyHtml = (()=>{
    const txtValue = normalizeValue(input.value)
    const labels = ['Referred By','Indications','Consent','Procedure','Pre-Medication','Rectal Exam','Findings','Impression','Recommendations']
    const set = new Set(labels)
    const sections: Record<string,string> = {}
    let cur = ''
    let buf: string[] = []
    function push(){ if (cur){ sections[cur] = (buf.join('\n')).trim(); buf = [] } }
    for (const raw of String(txtValue||'').split(/\r?\n/)){
      const line = raw.trim()
      if (set.has(line)){
        if (!cur){ buf = [] } else { push() }
        cur = line; continue
