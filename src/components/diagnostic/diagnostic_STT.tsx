...
type Row = { parameter: string; normalRange: string; result: string; flag: string }
...
function safeParse(value: string): FormData | null {
  try {
    ...
    if (!rows) return { rows: [], clinicalNote }
    const normRows: Row[] = rows.map((r: any) => ({
...
const DEFAULT_ROWS: Row[] = [
  { parameter: 'Er. T', normalRange: '3.24-9.81', result: '', flag: '' },
  { parameter: 'Ej. T', normalRange: '2.21-2.71', result: '', flag: '' },
]

export default function Diagnostic_STT({ value, onChange }: Props){
          ...
          <tbody>
            {rows.map((r, idx) => (
              <tr key={r.parameter} className="border-t border-slate-100">
                ...
                <td className="px-3 py-2 text-slate-700">{r.normalRange}</td>
