...
export async function printUltrasoundReport(input: {
  ...
  const bodyHtml = (()=>{
    ...
    push()
    const imgsFromParam = Array.isArray(input.images) ? input.images : []
    const imgsFromText = (sections['Images']||'').split(/\r?\n/).map(s=>s.trim()).filter(Boolean)
    const imgs = (imgsFromParam.length ? imgsFromParam : imgsFromText).filter(Boolean)
    let html = `<div class="title-mid">ULTRASOUND REPORT</div><div class="box">`
    for (const key of labels){
      const val = (sections as any)[key]
      if (key==='Images'){
        if (!imgs.length) continue
        html += `<div class="sec"><div class="sec-title">${esc(key)}</div>`
        html += `<div class="img-grid">${imgs.map((src,i)=>`<div class=\"img-wrap\"><img class=\"img\" src=\"${esc(src)}\" alt=\"img-${i}\"/></div>`).join('')}</div>`
        html += `</div>`
        continue
      }
  ...
  const consultHtml = consultants.length ? `<div class=\"consult-grid\" style=\"--cols:${consultants.length}\">${consultants.map(c=>`<div class=\\\"consult\\\"><div class=\\\"name\\\">${esc(c.name||'')}</div><div class=\\\"deg\\\">${esc(c.degrees||'')}</div><div class=\\\"title\\\">${esc(c.title||'')}</div></div>`).join('')}</div>` : ''
  ...
  <style>
    ...
    .img-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:10px;margin-top:8px}
    .img-wrap{border:1px solid #e2e8f0;border-radius:10px;padding:6px;page-break-inside:avoid;break-inside:avoid}
    .img{display:block;width:100%;height:auto;object-fit:contain;max-height:380px}
    ...
    .consult-grid{display:grid;grid-template-columns:repeat(var(--cols,3),1fr);gap:4px 18px;margin-top:6px}
