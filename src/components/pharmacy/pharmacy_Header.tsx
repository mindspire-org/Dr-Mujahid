...
export default function Pharmacy_Header({ onToggleSidebar, collapsed, onToggleTheme, theme }: Props) {
  ...
  function handleToggleTheme(){
    ...
    try { localStorage.setItem('pharmacy.theme', next) } catch {}
    try { document.documentElement.classList.toggle('dark', next === 'dark') } catch {}
    try { const scope = document.querySelector('.pharmacy-scope'); if (scope) scope.classList.toggle('dark', next === 'dark') } catch {}
    onToggleTheme?.()
  }
  return (
    <header
    ...
    >
      <div className="flex h-full items-center">
        ...
        <div className="flex h-full flex-1 items-center gap-0 px-3 sm:px-5">
          ...
          <Link to="/pharmacy" className="ml-3 flex items-center gap-2">
            ...
            <div className="font-semibold text-white">{pharmacyName || 'Pharmacy'}</div>
          ...
          <div className="ml-auto flex items-center gap-3 text-sm">
            ...
            <div className="rounded-md border border-white/15 px-3 py-1.5 text-white">{displayName || 'admin'}</div>
          </div>
        </div>
      </div>
    </header>
