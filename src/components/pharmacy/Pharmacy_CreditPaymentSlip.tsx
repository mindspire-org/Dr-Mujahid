...
export type PharmacyCreditPaymentSlipData = {
  customerName: string
  phone?: string
  createdAt: string
  amount: number
...
export default function Pharmacy_CreditPaymentSlip({ open, onClose, data, autoPrint = false, user }: {
  open: boolean
  onClose: () => void
  data: PharmacyCreditPaymentSlipData
  autoPrint?: boolean
  user?: string
}) {
  const [settings, setSettings] = useState({ name: 'Pharmacy', phone: '', address: '', logoDataUrl: '', slipFooter: 'Powered by Hospital MIS' })
  ...
  useEffect(() => {
    ...
    ;(async () => {
      try {
        const s = await pharmacyApi.getSettings() as any
        if (!mounted) return
        setSettings({
          name: s?.pharmacyName || s?.name || 'Pharmacy',
          phone: s?.phone || '',
          address: s?.address || '',
          logoDataUrl: s?.logoDataUrl || '',
          slipFooter: s?.reportFooter || s?.slipFooter || 'Powered by Hospital MIS',
        })
      } catch {}
    })()
    ...
    async function loadPayTo() {
      try {
        ...
        const add = (code?: string, nameOnly?: string) => {
          ...
          if (!c || !nameOnly) return
          map[c] = String(nameOnly).trim()
        }
  ...
  const payedToName = normalizePayToLabel((receivedCode && payToNameMap[receivedCode]) ? payToNameMap[receivedCode] : (data.receptionistName || ''))

  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-2 sm:p-4 print:bg-white print:static">
      <div id="pharmacy-credit-slip" className="w-full max-w-[384px] max-h-[calc(100vh-1rem)] overflow-y-auto rounded-md border border-slate-300 bg-white p-4 shadow print:shadow-none print:border-0 print:w-[300px] print:max-h-none print:overflow-visible">
        ...
        <div className="space-y-1 text-sm text-slate-800">
          <Row label="Customer:" value={data.customerName || '-'} boldValue />
      ...
      <style>{`
        ...
        #pharmacy-credit-slip { font-family: 'Poppins', Arial, sans-serif }
        @media print {
          @page { size: 58mm auto; margin: 0 }
          html, body { -webkit-print-color-adjust: exact; print-color-adjust: exact }
          body * { visibility: hidden !important }
          #pharmacy-credit-slip, #pharmacy-credit-slip * { visibility: visible !important }
          #pharmacy-credit-slip { position: absolute !important; left: 0; right: 0; top: 0; margin: 0 auto !important; padding: 10px 10px 0 10px !important; width: 300px !important; box-sizing: border-box !important; line-height: 1.45; overflow: visible !important; z-index: 2147483647; font-size: 14px !important }
          #pharmacy-credit-slip, #pharmacy-credit-slip * { color: #000 !important }
          .print\\:hidden { display: none !important }
          #pharmacy-credit-slip .text-xs { font-size: 13px !important }
          #pharmacy-credit-slip .text-sm { font-size: 14px !important }
          #pharmacy-credit-slip .text-lg { font-size: 18px !important }
