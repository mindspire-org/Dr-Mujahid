...
type Props = {
  ...
  onReject?: (draftId: string, lineId?: string)=>void
  onApproveAll?: ()=>void
  onRejectAll?: ()=>void
...
export default function Pharmacy_InventoryTable({ rows = [], pending = false, onApprove, onReject, onApproveAll, onRejectAll, onView, onEdit, onDelete, onEditDraft, page, totalPages, limit, onChangeLimit, onPrev, onNext }: Props) {
                ...
                <td className="px-4 py-2">
                  {pending ? (
                    <div className="flex gap-2">
                      ...
                      <button onClick={()=> r.draftId && onReject?.(r.draftId, r.lineId)} className="rounded-md bg-rose-600 px-2 py-1 text-xs text-white hover:bg-rose-700">Reject</button>
                    </div>
                  ) : (
                    <div className="flex gap-2">
                      <button
                        onClick={()=> onView?.(String(r.medicine||'').trim().toLowerCase())}
