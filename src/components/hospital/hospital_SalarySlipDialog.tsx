...
export default function Hospital_SalarySlipDialog({ open, onClose, expense, staffName, month, basicSalary, netSalary, paidToDate }: Props){
  ...
  const [dbExpense, setDbExpense] = useState<SalarySlipExpense | null>(null)
  ...
  useEffect(()=>{
    ...
    if (!id) { setDbExpense(null); return }
    let mounted = true
    ;(async()=>{
      try {
        const res: any = await (hospitalApi as any).getExpense(id)
        if (!mounted) return
        const row = res?.expense || res
        setDbExpense(row || null)
      } catch {
        if (!mounted) return
        setDbExpense(null)
      }
  ...
  const effectiveExpense = useMemo(()=> (dbExpense || expense), [dbExpense, expense])

  const eid = useMemo(()=> String((effectiveExpense?._id || effectiveExpense?.id) || ''), [effectiveExpense])
  const createdBy = useMemo(()=> String(effectiveExpense?.createdBy || ''), [effectiveExpense])
  const staffId = useMemo(()=> String((effectiveExpense as any)?.staffId || ''), [effectiveExpense])
  const salaryMonth = useMemo(()=> String((effectiveExpense as any)?.salaryMonth || ''), [effectiveExpense])
  const paymentMode = useMemo(()=> String((effectiveExpense as any)?.paymentMode || ''), [effectiveExpense])
  const parsedFromNote = useMemo(()=>{
    const note = String(effectiveExpense?.note || effectiveExpense?.description || '')
    const m = note.match(/Salary\s*\((?:full|half|custom)\)\s*for\s*(.+?)\s*—\s*(\d{4}-\d{2})/i)
    return { staff: m?.[1]?.trim(), mon: m?.[2] }
  }, [effectiveExpense])
  const staff = useMemo(()=> staffName || parsedFromNote.staff || staffId || '', [staffName, parsedFromNote, staffId])
  const mon = useMemo(()=> month || salaryMonth || parsedFromNote.mon || '', [month, salaryMonth, parsedFromNote])
  const dateStr = useMemo(()=> {
    if ((effectiveExpense as any)?.date) return (effectiveExpense as any).date
    if (effectiveExpense?.dateIso) return effectiveExpense.dateIso
    if (effectiveExpense?.datetime) try { return new Date(effectiveExpense.datetime).toISOString().slice(0,10) } catch {}
    return new Date().toISOString().slice(0,10)
  }, [effectiveExpense])
  const paid = useMemo(()=> Number(effectiveExpense?.amount || 0), [effectiveExpense])
          ...
          <div className="max-h-[75vh] overflow-y-auto px-6 py-6 print:p-0 print:overflow-visible">
            <div id="hospital-salary-slip" className="mx-auto w-[440px] print:w-[440px]">
              ...
              <div className="mt-2 text-xs text-slate-700 print:text-black space-y-1">
                ...
                {paymentMode ? (<div className="flex items-center justify-between"><div>Payment</div><div className="capitalize">{paymentMode}</div></div>) : null}
              ...
              {(effectiveExpense?.note || effectiveExpense?.description) ? (
                <div className="mt-3 border-t border-dashed pt-2 text-xs text-slate-700 break-words print:text-black">{effectiveExpense?.note || effectiveExpense?.description}</div>
              ) : null}
