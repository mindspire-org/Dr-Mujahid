...
export type HospitalCreditPaymentSlipData = {
  patientName: string
  mrn?: string
  phone?: string
  createdAt: string
...
export default function Hospital_CreditPaymentSlip({ open, onClose, data, autoPrint = false, user }: { open: boolean; onClose: ()=>void; data: HospitalCreditPaymentSlipData; autoPrint?: boolean; user?: string }){
  const [settings, setSettings] = useState({ name: 'Hospital', phone: '', address: '', logoDataUrl: '', slipFooter: 'Powered by Hospital MIS' })
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-2 sm:p-4 print:bg-white print:static">
      <div id="hospital-credit-slip" className="w-full max-w-[384px] max-h-[calc(100vh-1rem)] overflow-y-auto rounded-md border border-slate-300 bg-white p-4 shadow print:shadow-none print:border-0 print:w-[300px] print:max-h-none print:overflow-visible">
      ...
      <style>{`
        ...
        #hospital-credit-slip { font-family: 'Poppins', Arial, sans-serif }
        @media print {
          @page { size: 58mm auto; margin: 0 }
          html, body { -webkit-print-color-adjust: exact; print-color-adjust: exact }
          body * { visibility: hidden !important }
          #hospital-credit-slip, #hospital-credit-slip * { visibility: visible !important }
          #hospital-credit-slip { position: absolute !important; left: 0; right: 0; top: 0; margin: 0 auto !important; padding: 10px 10px 0 10px !important; width: 300px !important; box-sizing: border-box !important; line-height: 1.45; overflow: visible !important; z-index: 2147483647; font-size: 14px !important }
          #hospital-credit-slip, #hospital-credit-slip * { color: #000 !important }
          .print\:hidden { display: none !important }
          #hospital-credit-slip .text-xs{ font-size: 13px !important }
          #hospital-credit-slip .text-sm{ font-size: 14px !important }
          #hospital-credit-slip .text-lg{ font-size: 18px !important }
