...
export const hospitalSidebarNavTop: NavItem[] = [
  ...
  { to: '/hospital/credit-patients', label: 'Credit Patients', icon: Users },
  { to: '/hospital/departments', label: 'Departments', icon: Building2 },
]

export const hospitalSidebarNavBottom: NavItem[] = [
  ...
  { to: '/hospital/finance/petty-cash-balance', label: 'Manage Petty Cash', icon: Wallet },
  { to: '/hospital/finance/manage-bank-balance', label: 'Manage Bank Balance', icon: Wallet },
  { to: '/hospital/notifications', label: 'Notifications', icon: Bell },
  { to: '/hospital/user-management', label: 'User Mangement', icon: UserCog },
...
export const hospitalSidebarGroups: { label: string; icon: LucideIcon; items: NavItem[] }[] = [
  {
...
export default function Hospital_Sidebar({
  ...
  async function logout() {
    ...
    try {
      ...
      // Clear all potential portal sessions for security
      const portals = [
  ...
  const renderNav = ({ isCollapsed }: { isCollapsed: boolean }) => {
    return (
      <nav className="hospital-sidebar-scroll flex-1 min-h-0 overflow-y-auto p-3 space-y-1">
        ...
        {[...hospitalSidebarNavBottom].filter(i => isAllowed(i.to)).map(item => {
          const Icon = item.icon
          return (
            <NavLink
              key={item.to}
