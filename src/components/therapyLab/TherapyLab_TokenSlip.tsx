  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-2 sm:p-4 print:bg-white print:static">
      <div
        ...
        className="w-full max-w-[384px] max-h-[calc(100vh-1rem)] overflow-y-auto rounded-md border border-slate-300 bg-white p-4 shadow print:shadow-none print:border-0 print:w-[300px] print:max-h-none print:overflow-visible"
      >
        ...
        {(data.amountReceived != null ||
          ...
          <>
            ...
            <div className="space-y-1 text-sm text-slate-800">
              {data.duesBefore != null && <Row label="Previous Dues:" value={`PKR ${Number(data.duesBefore || 0).toLocaleString()}`} />}
              {data.advanceBefore != null && <Row label="Previous Advance:" value={`PKR ${Number(data.advanceBefore || 0).toLocaleString()}`} />}
              {data.useAdvance && data.advanceApplied != null && <Row label="Advance Used:" value={`PKR ${Number(data.advanceApplied || 0).toLocaleString()}`} />}
              {data.amountReceived != null && <Row label="Received Now:" value={`PKR ${Number(data.amountReceived || 0).toLocaleString()}`} boldValue />}
              {data.duesAfter != null && <Row label="Dues After:" value={`PKR ${Number(data.duesAfter || 0).toLocaleString()}`} />}
              {data.advanceAfter != null && <Row label="Advance After:" value={`PKR ${Number(data.advanceAfter || 0).toLocaleString()}`} />}
            </div>
          </>
        )}
