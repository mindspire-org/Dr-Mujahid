...
export default function PrescriptionPrint({ printId = 'prescription-print', doctor, settings, patient, items, labTests, labNotes, diagnosticTests, diagnosticNotes, therapyTests, therapyNotes, primaryComplaint, primaryComplaintHistory, familyHistory, treatmentHistory, allergyHistory, history, examFindings, diagnosis, advice, createdAt }: Props){
        ...
        <div className="grid grid-cols-12 gap-4">
          <div className="col-span-5 space-y-3">
            ...
            <div>
              ...
              <div className="min-h-[40px] whitespace-pre-wrap rounded-md border border-slate-300 p-2 text-sm">{primaryComplaint && primaryComplaint.trim() ? primaryComplaint : 'N/A'}</div>
            </div>
            <div>
              <div className="text-xs font-semibold text-slate-700">History of Primary Complaint</div>
              <div className="min-h-[40px] whitespace-pre-wrap rounded-md border border-slate-300 p-2 text-sm">{primaryComplaintHistory && primaryComplaintHistory.trim() ? primaryComplaintHistory : 'N/A'}</div>
            </div>
            <div>
              <div className="text-xs font-semibold text-slate-700">Family History</div>
              <div className="min-h-[56px] whitespace-pre-wrap rounded-md border border-slate-300 p-2 text-sm">{familyHistory && familyHistory.trim() ? familyHistory : 'N/A'}</div>
            </div>
            <div>
              <div className="text-xs font-semibold text-slate-700">Allergy History</div>
              <div className="min-h-[56px] whitespace-pre-wrap rounded-md border border-slate-300 p-2 text-sm">{allergyHistory && allergyHistory.trim() ? allergyHistory : 'N/A'}</div>
            </div>
            <div>
              <div className="text-xs font-semibold text-slate-700">Treatment History</div>
              <div className="min-h-[56px] whitespace-pre-wrap rounded-md border border-slate-300 p-2 text-sm">{treatmentHistory && treatmentHistory.trim() ? treatmentHistory : 'N/A'}</div>
            </div>
            <div>
              <div className="text-xs font-semibold text-slate-700">Medical History</div>
              <div className="min-h-[56px] whitespace-pre-wrap rounded-md border border-slate-300 p-2 text-sm">{history && history.trim() ? history : 'N/A'}</div>
            </div>
            <div>
              <div className="text-xs font-semibold text-slate-700">Examination</div>
              <div className="min-h-[40px] whitespace-pre-wrap rounded-md border border-slate-300 p-2 text-sm">{examFindings && examFindings.trim() ? examFindings : 'N/A'}</div>
            </div>
            <div>
              <div className="text-xs font-semibold text-slate-700">Diagnosis / Disease</div>
              <div className="min-h-[32px] whitespace-pre-wrap rounded-md border border-slate-300 p-2 text-sm">{diagnosis && diagnosis.trim() ? diagnosis : 'N/A'}</div>
            </div>
            <div>
              <div className="text-xs font-semibold text-slate-700">Advice</div>
              <div className="min-h-[32px] whitespace-pre-wrap rounded-md border border-slate-300 p-2 text-sm">{advice && advice.trim() ? advice : 'N/A'}</div>
            </div>
            <div>
              <div className="text-xs font-semibold text-slate-700">Lab Tests</div>
              <div className="min-h-[32px] whitespace-pre-wrap rounded-md border border-slate-300 p-2 text-sm">
                <div>{Array.isArray(labTests) && labTests.length > 0 ? labTests.join(', ') : 'N/A'}</div>
                {labNotes && labNotes.trim() && <div className="mt-1 text-xs text-slate-600">Note: {labNotes}</div>}
              </div>
            </div>
            <div>
              <div className="text-xs font-semibold text-slate-700">Diagnostic Tests</div>
              <div className="min-h-[32px] whitespace-pre-wrap rounded-md border border-slate-300 p-2 text-sm">
                <div>{Array.isArray(diagnosticTests) && diagnosticTests.length > 0 ? diagnosticTests.join(', ') : 'N/A'}</div>
                {diagnosticNotes && diagnosticNotes.trim() && <div className="mt-1 text-xs text-slate-600">Note: {diagnosticNotes}</div>}
              </div>
            </div>
            <div>
              <div className="text-xs font-semibold text-slate-700">Therapy</div>
              <div className="min-h-[32px] whitespace-pre-wrap rounded-md border border-slate-300 p-2 text-sm">
                <div>{Array.isArray(therapyTests) && therapyTests.length > 0 ? therapyTests.join(', ') : 'N/A'}</div>
                {therapyNotes && therapyNotes.trim() && <div className="mt-1 text-xs text-slate-600">Note: {therapyNotes}</div>}
              </div>
            </div>
          </div>
