  ...
  const onSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const canReception = (() => {
        ...
        const arr = (permissions as any)?.reception
        return Array.isArray(arr) && arr.length > 0
      })()

      if (!canReception && String(user.role) !== "Reception") {
      ...
      try {
        localStorage.setItem(
          'reception.session',
          JSON.stringify({ username: user.username, role: user.role, permissions })
        )
      } catch { }
      ...
      try {
        if (token) {
          ...
          localStorage.setItem('reception.token', String(token))
        }
      } catch { }
