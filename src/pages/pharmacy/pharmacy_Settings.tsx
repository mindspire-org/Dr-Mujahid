...
export default function Pharmacy_Settings() {
  ...
  useEffect(() => {
    ...
    ;(async () => {
      try {
        const s = await pharmacyApi.getSystemSettings() as any
        if (!mounted || !s) return
  ...
  const savePharmacy = async () => {
    try {
      ...
      showToast('success', 'Pharmacy settings saved')
    } catch (e) {
      console.error(e)
      showToast('error', 'Failed to save pharmacy settings')
    }
  }
  ...
  const saveSystem = () => {
    ;(async()=>{
      try {
        await pharmacyApi.updateSystemSettings({ taxRate, discountRate, currency, dateFormat })
    ...
    <div className="space-y-4">
      {toast && (
        <div className="fixed bottom-4 right-4 z-[60]">
          <div className={`rounded-lg px-4 py-3 text-sm shadow-lg ring-1 ${toast.type==='success' ? 'bg-emerald-600 text-white ring-emerald-700/30' : 'bg-rose-600 text-white ring-rose-700/30'}`}>
            {toast.message}
          </div>
        </div>
      )}
