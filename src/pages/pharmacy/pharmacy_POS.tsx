...
export default function Pharmacy_POS() {
  ...
  useEffect(() => {
    ...
    ;(async () => {
      try {
        const res: any = await pharmacyApi.listInventory({ search: query || undefined, company: company || undefined, page, limit: rowsPerPage })
        if (!mounted) return
        try {
          const all: any = await pharmacyApi.listInventory({ page: 1, limit: 2000 })
          const list: any[] = all?.items ?? all ?? []
          const opts = Array.from(new Set(list.map((it:any)=> String(it.company || '').trim()).filter(Boolean))).sort()
          if (mounted) setCompanyOptions(opts)
        } catch {}
  ...
  }, [query, company, page, rowsPerPage])
    ...
    const onKeyDown = (e: KeyboardEvent) => {
      ...
      if (searchOpen && searchFocused) {
        ...
        if (e.key === 'Enter' && !e.shiftKey) {
          ...
          if (item) {
            e.preventDefault()
            addToCart(item.id, { focusQty: false })
            setQuery('')
            setSearchOpen(false)
  ...
  useEffect(() => {
    const handler = (e: KeyboardEvent) => {
      ...
      // Allow scanner handling even if qty input is focused; block normal typing into qty input
      if (isTyping && !isQtyInput) return
      if (isQtyInput) { try { e.preventDefault(); e.stopPropagation() } catch {} }
      const now = Date.now()
      ...
      <div className="lg:col-span-2 space-y-3">
          <div className="flex flex-col gap-3">
            <div className="relative w-full">
              <input
                ...
                className="w-full rounded-2xl border border-slate-300 px-5 py-4 pr-14 text-lg shadow-sm outline-none transition focus:border-sky-500 focus:ring-4 focus:ring-sky-200/60 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-100 dark:placeholder-slate-400 dark:focus:border-sky-500 dark:focus:ring-sky-900/40"
                placeholder="Search / scan barcode…"
              />

              <button type="button" onClick={clear} className="absolute right-2 top-1/2 -translate-y-1/2 rounded-xl border border-slate-200 bg-white p-2 text-rose-600 shadow-sm hover:bg-rose-50 dark:border-slate-700 dark:bg-slate-900 dark:text-rose-300 dark:hover:bg-slate-800" aria-label="Clear cart" title="Clear cart">
                <Trash2 className="h-5 w-5" />
              </button>

              {searchOpen && suggestions.length > 0 && (
                <div className="absolute left-0 right-0 top-[calc(100%+8px)] z-30 overflow-hidden rounded-xl border border-slate-200 bg-white shadow-2xl dark:border-slate-700 dark:bg-slate-900">
                  {suggestions.map((p, i) => (
                    <button type="button" key={p.id} onMouseEnter={() => setSuggestionSel(i)} onClick={() => { addToCart(p.id, { focusQty: false }); setQuery(''); setSearchOpen(false); try { searchInputRef.current?.focus(); searchInputRef.current?.select() } catch {} }} className={`relative flex w-full items-center gap-3 px-4 py-3 text-left text-sm transition ${i === suggestionSel ? 'bg-sky-100/80 text-slate-900 ring-1 ring-sky-200 dark:bg-sky-900/30 dark:text-slate-100' : 'hover:bg-slate-50 dark:hover:bg-slate-800'}`}>
                      ...
                      <div className="min-w-0 flex-1">
                        ...
                        <div className="truncate text-xs text-slate-500 dark:text-slate-400">{p.company ? `${p.company} • ` : ''}{p.genericName || ''}</div>
                      </div>
                      <div className="shrink-0 text-right">
                        <div className="text-xs font-semibold text-slate-700 dark:text-slate-300">PKR {p.unitPrice.toFixed(2)}</div>
                        <div className={`text-[11px] ${p.stock <= 0 ? 'text-rose-600' : 'text-emerald-700 dark:text-emerald-400'}`}>{p.stock <= 0 ? 'Out' : `Stock ${p.stock}`}</div>
                      </div>
                    </button>
                  ))}
                </div>
            ...
            <div className="flex flex-wrap items-center gap-2">
              ...
              <div className="flex flex-wrap items-center gap-2">
                <select value={company} onChange={e=>{ setCompany(e.target.value); setPage(1) }} className="rounded-xl border border-slate-300 px-3 py-2 text-sm text-slate-700">
          ...
          {view==='grid' ? (
            <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
              {visible.map((p) => (
                <div key={p.id} className={`rounded-xl border border-slate-200 bg-white p-4 dark:border-slate-700 dark:bg-slate-900`}>
                  <div className="flex items-start justify-between">
                    <div>
                      <div className="font-semibold text-slate-800 capitalize">{p.name}</div>
                      {p.company ? <div className="text-xs text-slate-500 capitalize">Brand: {p.company}</div> : null}
            ...
            <div className="divide-y divide-slate-200 rounded-xl border border-slate-200 bg-white dark:divide-slate-700 dark:border-slate-700 dark:bg-slate-900">
              {visible.map((p, i) => (
                <div key={p.id} className={`flex items-center gap-3 p-3 ${i===sel? 'bg-navy-50' : ''}`}>
                  <div className="flex-1 min-w-0">
                    ...
                    {p.company ? <div className="text-xs text-slate-500 capitalize truncate">Brand: {p.company}</div> : null}
