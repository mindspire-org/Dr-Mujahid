...
export default function Pharmacy_POS() {
  ...
  const [sys, setSys] = useState<{ taxRate: number; discountRate: number; currency: string; dateFormat: string }>({ taxRate: 0, discountRate: 0, currency: 'PKR', dateFormat: 'DD/MM/YYYY' })
  ...
  // Restore held sale from Hold Sales page
  useEffect(() => {
    const applyHoldDoc = (doc: any) => {
      try {
    ...
    // sessionStorage-based restore (for navigation from Hold Sales)
    try {
      const raw = sessionStorage.getItem('pharmacy.hold-sale.restore')
      if (raw) {
        sessionStorage.removeItem('pharmacy.hold-sale.restore')
        const doc = JSON.parse(raw)
        applyHoldDoc(doc)
      }
    } catch {}

    const onRestore = (ev: any) => {
      applyHoldDoc(ev?.detail || {})
    }
    window.addEventListener('pharmacy:hold-sale:restore' as any, onRestore as any)
    return () => window.removeEventListener('pharmacy:hold-sale:restore' as any, onRestore as any)
  }, [])
  ...
  useEffect(() => {
    ...
    ;(async () => {
      try {
        const s: any = await pharmacyApi.getSystemSettings()
        if (!mounted || !s) return
        const next = {
          taxRate: Number(s.taxRate || 0),
          discountRate: Number(s.discountRate || 0),
          currency: String(s.currency || 'PKR'),
          dateFormat: String(s.dateFormat || 'DD/MM/YYYY'),
        }
        setSys(next)
        setBillDiscountPct(prev => (prev === 0 ? next.discountRate : prev))
      } catch {}
    })()
  ...
  const totals = useMemo(() => {
    ...
    const taxPct = Math.max(0, Math.min(100, Number(sys.taxRate || 0)))
    const tax = Math.max(0, (subtotal - lineDiscount - billDiscount) * (taxPct / 100))
    const total = subtotal - lineDiscount - billDiscount + tax
    return { subtotal, lineDiscount, billDiscount, tax, total }
  }, [cart, billDiscountPct, productIndex, sys.taxRate])
                  ...
                  {suggestions.map((p, i) => (
                    <button type="button" key={p.id} onMouseEnter={() => setSuggestionSel(i)} onClick={() => { addToCart(p.id, { focusQty: false }); setQuery(''); setSearchOpen(false); try { searchInputRef.current?.focus(); searchInputRef.current?.select() } catch {} }} className={`relative flex w-full items-center gap-3 px-4 py-3 text-left text-sm transition ${i === suggestionSel ? 'bg-sky-100/80 text-slate-900 ring-1 ring-sky-200' : 'hover:bg-slate-50'}`}>
                      ...
                      <div className="shrink-0 text-right">
                        <div className="text-xs font-semibold text-slate-700">{sys.currency} {p.unitPrice.toFixed(2)}</div>
            ...
            <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
              {visible.map((p) => {
                ...
                return (
                  ...
                  <div className="mt-1 text-xs text-slate-500">Sale/Pack: {sys.currency} {p.salePerPack.toFixed(2)}</div>
                  <div className="text-xs text-slate-500">Units/Pack: {p.unitsPerPack}</div>
                  <div className="mt-3 text-lg font-semibold text-slate-900">{sys.currency} {p.unitPrice.toFixed(2)}</div>
                  <div className="mt-3">
                ...
                return (
                  <div key={p.id} className={`flex items-center gap-3 p-3 ${
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="text-xs text-slate-500">Sale/Pack: {sys.currency} {p.salePerPack.toFixed(2)} · Units/Pack: {p.unitsPerPack}</div>
                    </div>
                    <div className="shrink-0 text-sm font-semibold text-slate-800 w-24 text-right">{sys.currency} {p.unitPrice.toFixed(2)}</div>
      ...
      <div className="space-y-4">
        ...
        <div className="rounded-xl border border-slate-200 bg-white">
          ...
          <div className="space-y-2 p-4 text-sm text-slate-700 ">
            <div className="flex items-center justify-between"><span>Subtotal:</span><span>{sys.currency} {totals.subtotal.toFixed(2)}</span></div>
            <div className="flex items-center justify-between"><span>Line Discounts:</span><span>{sys.currency} {totals.lineDiscount.toFixed(2)}</span></div>
            ...
            <div className="flex items-center justify-between"><span>Sales Tax ({Number(sys.taxRate||0)}%):</span><span>{sys.currency} {totals.tax.toFixed(2)}</span></div>
            <div className="mt-2 flex items-center justify-between text-base font-semibold text-navy"><span>Total Amount:</span><span>{sys.currency} {totals.total.toFixed(2)}</span></div>
        ...
        {heldOpen && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-3 sm:p-4" role="dialog" aria-modal="true">
            <div className="w-full max-w-xl max-h-[85vh] overflow-hidden rounded-xl bg-white shadow-2xl">
              ...
              <div className="max-h-[calc(85vh-56px)] overflow-y-auto divide-y px-1">
                ...
                {heldBills.map(h=> (
                  <div key={String((h as any)._id||'')}
                    ...
                    <div className="min-w-0">
                      <div className="font-medium text-slate-800 break-all">{String((h as any)._id||'')}</div>
                    ...
                    <div className="flex items-center gap-2 sm:justify-end">
                      <button type="button" onClick={()=> restoreHeld(String((h as any)._id||''))} className="btn flex-1 sm:flex-none">Restore</button>
                      <button type="button" onClick={()=> deleteHeld(String((h as any)._id||''))} className="btn-outline-navy flex-1 sm:flex-none">Delete</button>
                    </div>
                  </div>
                ))}
              </div>
