  ...
  const onSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const canFinance = (() => {
        ...
        const arr = (permissions as any)?.finance
        return Array.isArray(arr) && arr.length > 0
      })()

      if (!u || (!canFinance && String(u.role) !== "Finance")) {
      ...
      try {
        localStorage.setItem(
          "finance.session",
          JSON.stringify({ username: u.username, role: u.role, permissions })
        );
        if (token) {
          localStorage.setItem('hospital.token', String(token))
          localStorage.setItem('token', String(token))
          localStorage.setItem('finance.token', String(token))
        }
      } catch { }
