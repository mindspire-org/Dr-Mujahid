      ...
      <div className="rounded-xl border border-slate-200 bg-white p-4">
        <div className="mb-3 flex flex-col gap-3 sm:flex-row sm:flex-wrap sm:items-center">
          ...
          <div className="flex flex-col gap-2 sm:ml-auto sm:flex-row sm:flex-wrap sm:items-center">
            <button onClick={() => setUpdateOpen(true)} className="btn w-full sm:w-auto"><RotateCw className="h-4 w-4" /> Update Stock</button>
            <button onClick={() => setAddInvoiceOpen(true)} className="btn w-full sm:w-auto"><CalendarDays className="h-4 w-4" /> Add Invoice</button>
            <button onClick={()=>setRefreshTick(t=>t+1)} className="btn-outline-navy w-full sm:w-auto"><RotateCw className="h-4 w-4" /> Refresh</button>
            <button onClick={exportCsv} className="btn-outline-navy w-full sm:w-auto"><FileDown className="h-4 w-4" /> Export CSV</button>
          </div>
        </div>
