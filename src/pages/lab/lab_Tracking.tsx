      ...
      <div className="rounded-xl border border-slate-200 bg-white p-4">
        ...
        <div className="mt-3 flex flex-col gap-2 sm:flex-row sm:flex-wrap sm:items-center">
          <div className="relative w-full sm:min-w-[260px] sm:flex-1">
            ...
            <input value={q} onChange={e=>{ setQ(e.target.value); setPage(1) }} placeholder="Search by sample ID, patient, or test..." className="w-full rounded-md border border-slate-300 pl-9 pr-3 py-2 outline-none focus:border-violet-500 focus:ring-2 focus:ring-violet-200" />
          ...
          <div className="flex w-full flex-wrap items-center gap-2 text-sm sm:w-auto">
            ...
            <input type="date" value={from} onChange={e=>{ setFrom(e.target.value); setPage(1) }} className="w-full rounded-md border border-slate-300 px-2 py-1 sm:w-auto" />
            <input type="date" value={to} onChange={e=>{ setTo(e.target.value); setPage(1) }} className="w-full rounded-md border border-slate-300 px-2 py-1 sm:w-auto" />
          </div>
          ...
          <div className="flex w-full flex-wrap items-center gap-2 text-sm sm:ml-auto sm:w-auto">
            <span>Rows</span>
            ...
            <button onClick={downloadRegister} className="w-full rounded-md border border-slate-300 px-3 py-1.5 text-slate-700 hover:bg-slate-50 sm:w-auto">Download Daily Register</button>
          </div>
        </div>
        {notice && (
                ...
                acc.push(
                  <tr key={`${o.id}-${tid}-${idx}`} className="border-b border-slate-100">
                    ...
                    <td className="px-4 py-2 whitespace-nowrap">
                      <input type="time" value={o.sampleTime || ''} onChange={e=>setSampleTimeFor(o.id, e.target.value)} className="w-[120px] rounded-md border border-slate-300 px-2 py-1" />
                    </td>
                    <td className="px-4 py-2 whitespace-nowrap">
                    ...
                    <td className="px-4 py-2 whitespace-nowrap">
                      <div className="flex flex-wrap items-center gap-2">
                        <button onClick={()=>printToken(o.id)} className="inline-flex items-center gap-1 rounded-md border border-slate-300 px-2 py-1 text-slate-700 hover:bg-slate-50"><Printer className="h-4 w-4" /> Print Token</button>
                        {rowStatus==='received' ? (
                          <button onClick={()=>openReturn(o.id, String(tid))} className="rounded-md bg-rose-600 px-2 py-1 text-xs font-medium text-white hover:bg-rose-700">Return</button>
                        ) : null}
                        {rowStatus==='returned' ? (
                          <button onClick={()=>openUndo(o.id, String(tid))} className="rounded-md bg-violet-600 px-2 py-1 text-xs font-medium text-white hover:bg-violet-700">Undo</button>
                        ) : null}
                        <button onClick={()=>requestDelete(o.id)} className="inline-flex items-center gap-1 rounded-md bg-rose-600 px-2 py-1 text-xs font-medium text-white hover:bg-rose-700"><Trash2 className="h-4 w-4" /> Delete</button>
                      </div>
                    </td>
                  </tr>
                )
