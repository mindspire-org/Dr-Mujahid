...
export default function Lab_Login() {
  ...
  const onSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res: any = await hospitalApi.loginHospitalUser(uname, password, 'lab')
      const user = res?.user
      ...
      if (!user || (!can('lab') && String(user?.role || '') !== 'Admin')) {
        setError('User role not permitted')
        return
      }
      try {
        if (token) {
          // Lab backend accepts hospital tokens (labAuth allows hospital scope if permissions include lab)
          localStorage.setItem('hospital.token', String(token))
          localStorage.setItem('lab.token', String(token))
          localStorage.setItem('token', String(token))
        }
        localStorage.setItem('hospital.session', JSON.stringify({ username: user.username, role: user.role, permissions }))
        localStorage.setItem('lab.session', JSON.stringify({ username: user.username, role: user.role, permissions }))
      } catch { }
      navigate('/lab')
    } catch (err: any) {
            ...
            <div className="relative p-8 pb-6 text-center">
              <div className="mx-auto mb-6 h-24 w-24 overflow-hidden rounded-3xl bg-gradient-to-br from-sky-400/20 to-blue-600/20 p-1 backdrop-blur-sm" style={{ animation: 'rotate3d 20s linear infinite' }}>
                <div className="h-full w-full overflow-hidden rounded-[1.3rem] bg-white/10 ring-1 ring-white/20">
                  <img src={`${(import.meta as any).env?.BASE_URL || '/'}mcclogo.png`} alt="Healthspire" className="h-full w-full object-cover" />
                </div>
              </div>
