...
export default function Lab_UserManagement() {
  ...
  useEffect(() => {
    ...
    ;(async () => {
      try {
        const res = await labApi.listUsers()
        if (!mounted) return
  ...
  const addUser = async () => {
    ...
    try {
      await labApi.createUser({ username: newUsername.trim(), password: newPassword, role: newRole })
  ...
  const performDelete = async () => {
    ...
    try { await labApi.deleteUser(id); setTick(prev=>prev+1); setNotice({ text: 'User deleted', kind: 'success' }) }
  ...
  const editUser = async (id: string) => {
    ...
    try { await labApi.updateUser(id, { username: name, password: pass, role }); setTick(prev=>prev+1) } catch (e) { console.error(e) }
  }
