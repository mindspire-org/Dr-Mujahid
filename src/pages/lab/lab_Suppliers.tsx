        ...
        {filtered.map(s => {
          ...
          return (
            <div key={s.id} className="rounded-xl border border-slate-200 bg-white p-4">
              ...
              {payOpen && (
                <div className="mt-3 grid gap-3 grid-cols-1 sm:grid-cols-2 lg:grid-cols-7">
                  <select value={paymentDraft.purchaseId || ''} onChange={e=>setPaymentDraft(p=>({ ...p, purchaseId: e.target.value || undefined }))} className="rounded-md border border-slate-300 px-3 py-2 text-sm sm:col-span-2 lg:col-span-2">
                  ...
                  <input value={paymentDraft.note} onChange={e=>setPaymentDraft(p=>({ ...p, note: e.target.value }))} placeholder="Note" className="rounded-md border border-slate-300 px-3 py-2 text-sm sm:col-span-2 lg:col-span-2" />
                  ...
                  <button onClick={()=>savePayment(s)} className="btn w-full lg:w-auto">Save</button>
                  <button onClick={cancelPayment} className="btn-outline-navy w-full lg:w-auto">Cancel</button>
                </div>
              )}
