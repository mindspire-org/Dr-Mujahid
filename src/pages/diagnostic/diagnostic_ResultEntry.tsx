...
export default function Diagnostic_ResultEntry(){
  ...
  useEffect(()=>{ let mounted = true; (async()=>{
    try {
      ...
      // Also do NOT filter by order.status for 'completed' because completion is tracked per-test (items[].status)
      // and the order.status might not be updated by Sample Tracking.
      const st = (status==='all' || status==='received' || status==='completed') ? undefined : status
  ...
  async function save(){
    ...
    try {
      await diagnosticApi.updateOrderItemTrack(selectedOrder.id, selectedTestId, { status: 'returned', reportingTime: payload.reportedAt })
    } catch {}
    ...
    setOrders(prev => prev.map(o => {
      ...
      if (idx>=0) items[idx] = { ...items[idx], status: 'returned', reportingTime: now }
      else items.push({ testId: selectedTestId, status: 'returned', reportingTime: now })
      return { ...o, items }
    }))
    // Clear selection
