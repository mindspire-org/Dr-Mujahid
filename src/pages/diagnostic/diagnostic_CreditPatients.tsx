...
import Diagnostic_CreditPaymentSlip, { type DiagnosticCreditPaymentSlipData } from '../../components/diagnostic/Diagnostic_CreditPaymentSlip'
...
function PayDialog({ open, onClose, row, payToAccounts, onPaid }: { open: boolean; onClose: () => void; row: CreditPatientRow | null; payToAccounts: Array<{ code: string; label: string }>; onPaid: (slip: DiagnosticCreditPaymentSlipData) => void }) {
  ...
  async function submit() {
    ...
    try {
      setBusy(true)
      const res: any = await diagnosticApi.payCreditPatient(row.patientId, {
        amount: amt,
