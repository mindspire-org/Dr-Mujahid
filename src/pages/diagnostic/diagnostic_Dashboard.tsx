...
export default function Diagnostic_Dashboard() {
  ...
  useEffect(() => {
    let mounted = true
      ; (async () => {
        try {
          const [summary, testsPage, weeklyOrders] = await Promise.all([
            diagnosticApi.getDashboardSummary({ from: effFrom, to: effTo }) as any,
    ...
    <div className="space-y-4">
      ...
      <div className="rounded-xl border border-slate-200 bg-white p-3">
        <div className="flex flex-wrap items-center gap-4 text-sm">
          ...
          <button onClick={() => { setFrom(''); setTo('') }} className="sm:ml-auto w-full sm:w-auto rounded-md border border-slate-300 px-3 py-1 hover:bg-slate-50">Reset</button>
        </div>
      </div>
