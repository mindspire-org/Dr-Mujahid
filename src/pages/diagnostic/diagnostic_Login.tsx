    ...
    try {
      ...
      try {
        if (token) {
          ...
          localStorage.setItem('diagnostic.token', String(token))
        ...
        localStorage.setItem('diagnostic.session', JSON.stringify(sess))
        localStorage.setItem("diagnostic.user", JSON.stringify(sess));
      } catch { }

      try { localStorage.setItem('diagnostic.sidebar.collapsed', '0') } catch { }
