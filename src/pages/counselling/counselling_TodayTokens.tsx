...
import Counselling_TokenSlip, { type CounsellingTokenSlipData } from '../../components/counselling/Counselling_TokenSlip'
...
export default function Counselling_TodayTokens() {
  ...
  async function load() {
    ...
    const res = await counsellingApi.listOrders({ from: today, to: today }) as any
    const items: TokenRow[] = (res.tokens || []).map((t: any) => ({
      _id: t._id,
      date: t.createdAt ? new Date(t.createdAt).toLocaleDateString('en-CA') : today,
  ...
  async function updateStatus(id: string, newStatus: 'Queued' | 'Completed') {
    try {
      setActioningId(id)
      await counsellingApi.updateSessionStatus(id, newStatus)
      await load()
  ...
  async function submitReturn() {
    ...
    try {
      setReturnBusy(true)
      await counsellingApi.returnOrder(returnRow._id, reason)
      setReturnOpen(false)
      setReturnRow(null)
      setReturnReason('')
      await load()
  ...
  async function submitEdit() {
    ...
    try {
      ...
      await counsellingApi.updateOrder(editRow._id, payload)
      setEditOpen(false)
      setEditRow(null)
      await load()
    } catch (e: any) {
  ...
  function printSlip(r: TokenRow) {
    const t = r.raw || {}
    const slip: CounsellingTokenSlipData = {
      tokenNo: r.tokenNo,
      ...
      tests: (t.packages || []).map((p: any) => ({ name: p.packageName || p.name, price: p.price })),
  ...
  const exportCSV = () => {
    ...
    a.href = url
    a.download = `counselling-tokens-${new Date().toISOString().slice(0, 10)}.csv`
    a.click()
              ...
              <tr key={r._id} className={`text-slate-700 ${r.status === 'returned' ? 'bg-amber-50' : ''}`}>
                ...
                <Td>
                  <div className="max-w-[200px] truncate">
                    {r.packages.map(p => p.packageName || p.name).join(', ') || '-'}
                  </div>
                </Td>
                ...
                <Td>
                  <div className="relative">
                    <button
                      onClick={() => setStatusMenuOpen(statusMenuOpen === r._id ? null : r._id)}
                    ...
                    {statusMenuOpen === r._id && (
                      <div className="absolute left-0 z-50 mt-1 w-32 rounded-md border border-slate-200 bg-white py-1 shadow-lg ring-1 ring-black/5">
      ...
      {returnOpen && returnRow && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 px-4 py-6">
          <div className="w-full max-w-md overflow-hidden rounded-xl bg-white shadow-2xl ring-1 ring-black/5">
            <div className="flex items-center justify-between border-b border-slate-200 px-5 py-3">
              <div className="text-base font-semibold text-slate-800">Return Token</div>
              <button onClick={() => { if (returnBusy) return; setReturnOpen(false); setReturnRow(null) }} className="text-slate-400 hover:text-slate-600"><X size={20} /></button>
            </div>
            <div className="p-5">
              <div className="mb-4 text-sm text-slate-600 text-center">Are you sure you want to return token #{returnRow.tokenNo}? This will mark it as returned and revert any financial impact.</div>
              <label className="mb-1 block text-xs font-medium text-slate-600">Reason of Return</label>
              <textarea value={returnReason} onChange={e => setReturnReason(e.target.value)} rows={3} placeholder="Enter reason for return..." className="w-full rounded-md border border-slate-300 px-3 py-2 text-sm focus:border-amber-500 focus:ring-2 focus:ring-amber-200 outline-none" />
              <div className="mt-6 flex gap-3">
                <button onClick={() => { setReturnOpen(false); setReturnRow(null) }} className="flex-1 rounded-md border border-slate-300 py-2 text-sm font-medium hover:bg-slate-50">Cancel</button>
                <button onClick={submitReturn} disabled={returnBusy || !returnReason.trim()} className="flex-1 rounded-md bg-amber-600 py-2 text-sm font-medium text-white hover:bg-amber-700 disabled:opacity-50">{returnBusy ? 'Processing...' : 'Confirm Return'}</button>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="max-h-[75vh] overflow-y-auto px-6 py-5 space-y-6">
          ...
          <section>
            ...
            <div className="mt-3 grid grid-cols-1 gap-3 sm:grid-cols-2">
              ...
              <Info label="Discount" value={`Rs. ${discount.toLocaleString()} (${t.discountType})`} />
