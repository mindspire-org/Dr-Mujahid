...
export default function Counselling_Login() {
  ...
  const onSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const permissions = (user?.permissions && typeof user.permissions === 'object') ? user.permissions : undefined
      ...
      try {
        if (token) {
          ...
          localStorage.setItem('counselling.token', String(token))
          localStorage.setItem('token', String(token))
        }
        localStorage.setItem('counselling.session', JSON.stringify({ username: user.username, role: user.role, permissions }))
        localStorage.setItem('hospital.session', JSON.stringify({ username: user.username, role: user.role, permissions }))
      } catch { }
    ...
    <div className={theme === 'dark' ? 'counselling-scope dark' : 'counselling-scope'}>
      <div className="min-h-screen bg-gradient-to-br from-[#0a1e3c] via-[#0b2b55] to-slate-900 flex items-center justify-center p-4 overflow-hidden relative">
        <div className="absolute inset-0 overflow-hidden">
          ...
          <div className="absolute -bottom-56 -left-56 h-[28rem] w-[28rem] rounded-full bg-teal-500/20 blur-3xl animate-pulse" style={{ animationDuration: '6s', animationDelay: '1s' }} />
          <div className="absolute left-1/2 top-1/2 h-[34rem] w-[34rem] -translate-x-1/2 -translate-y-1/2 rounded-full bg-sky-500/15 blur-3xl animate-pulse" style={{ animationDuration: '5s', animationDelay: '2s' }} />
          <div className="absolute top-20 left-20 h-32 w-32 bg-gradient-to-br from-emerald-400/10 to-teal-600/10 backdrop-blur-sm rounded-3xl" style={{ transform: 'rotateX(45deg) rotateZ(45deg)', animation: 'float 6s ease-in-out infinite' }} />
          <div className="absolute bottom-32 right-32 h-24 w-24 bg-gradient-to-br from-teal-400/10 to-emerald-600/10 backdrop-blur-sm rounded-2xl" style={{ transform: 'rotateX(-30deg) rotateZ(30deg)', animation: 'float 8s ease-in-out infinite', animationDelay: '1s' }} />
          <div className="absolute top-1/3 right-1/4 h-20 w-20 bg-gradient-to-br from-sky-400/10 to-teal-600/10 backdrop-blur-sm rounded-xl" style={{ transform: 'rotateX(60deg) rotateZ(-45deg)', animation: 'float 7s ease-in-out infinite', animationDelay: '2s' }} />
        </div>
        ...
        <div className="card-3d relative w-full max-w-md overflow-hidden rounded-3xl border border-white/10 bg-white/5 shadow-2xl backdrop-blur-2xl">
          ...
          <div className="relative">
            <div className="relative p-8 pb-6 text-center">
              ...
              <p className="text-sm text-white/60 font-medium">Counselling Portal</p>
            </div>
              ...
              <form onSubmit={onSubmit} className="space-y-5">
                <div>
                  ...
                  <div className="relative group">
                    ...
                    <input type="text" value={username} onChange={(e) => setUsername(e.target.value)} className="w-full rounded-2xl border-2 border-white/10 bg-white/5 backdrop-blur-sm px-12 py-3.5 text-white placeholder-white/40 outline-none transition-all focus:border-emerald-400/50 focus:bg-white/10 focus:ring-4 focus:ring-emerald-400/20" placeholder="Enter username" autoComplete="username" />
                  </div>
                </div>
                ...
                <div>
                  ...
                  <div className="relative group">
                    ...
                    <input type={showPassword ? 'text' : 'password'} value={password} onChange={(e) => setPassword(e.target.value)} className="w-full rounded-2xl border-2 border-white/10 bg-white/5 backdrop-blur-sm px-12 py-3.5 pr-14 text-white placeholder-white/40 outline-none transition-all focus:border-teal-400/50 focus:bg-white/10 focus:ring-4 focus:ring-teal-400/20" placeholder="Enter password" autoComplete="current-password" />
                ...
                {success && (
                  <div className="rounded-2xl border-2 border-emerald-400/30 bg-emerald-500/10 backdrop-blur-sm px-4 py-3 text-sm text-emerald-200 font-medium">{success}</div>
                )}

                <button type="submit" className="group relative mt-2 w-full overflow-hidden rounded-2xl bg-gradient-to-r from-emerald-500 via-teal-500 to-sky-600 px-4 py-4 font-bold text-white shadow-2xl shadow-emerald-500/30 transition-all hover:shadow-emerald-500/50 hover:scale-[1.02] focus:outline-none focus:ring-4 focus:ring-emerald-400/50">
                  <div className="absolute inset-0 bg-gradient-to-r from-emerald-400 via-teal-400 to-sky-500 opacity-0 transition-opacity group-hover:opacity-100" />
                ...
                <button
                  type="button"
                  onClick={() => navigate('/')}
                  className="mt-3 w-full rounded-2xl border-2 border-white/20 bg-white/5 px-4 py-3 font-semibold text-white/90 hover:text-white hover:bg-white/10 focus:outline-none focus:ring-4 focus:ring-emerald-400/30 transition-colors flex items-center justify-center gap-2"
                >
