...
const navTop: NavItem[] = [
  ...
  { to: '/counselling/appointments', label: 'Appointments', icon: CalendarDays },
  { to: '/counselling/history', label: 'History', icon: History },
]
...
export default function Counselling_Sidebar({
  ...
  useEffect(() => {
    ...
    // noop for now; kept for parity with other sidebars
  }, [collapseSignal])

  async function logout() {
    try {
      const raw = localStorage.getItem('hospital.session') || localStorage.getItem('counselling.session')
      const u = raw ? JSON.parse(raw) : null
      await hospitalApi.logoutHospitalUser(u?.username || '')
    } catch { }
    try {
      localStorage.removeItem('token')
      localStorage.removeItem('counselling.session')
      localStorage.removeItem('counselling.token')
    } catch { }
  ...
  const renderNav = ({ isCollapsed }: { isCollapsed: boolean }) => {
    return (
      <nav className="hospital-sidebar-scroll flex-1 min-h-0 overflow-y-auto p-3 space-y-1">
        {navTop.filter(i => isAllowed(i.to)).map(item => {
