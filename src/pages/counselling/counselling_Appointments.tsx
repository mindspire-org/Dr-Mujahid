...
export default function Counselling_Appointments() {
  ...
  const load = async () => {
    setLoading(true)
    try {
      const res = (await counsellingApi.listAppointments({ q, from, to, limit: 200 })) as any
  ...
  useEffect(() => {
    let mounted = true
      ; (async () => {
        try {
          const res = await counsellingApi.listPackages({ limit: 1000 }) as any
  ...
  const handleCreate = async (f: AppointmentForm) => {
    try {
      ...
      await counsellingApi.createAppointment(payload)
      setCreateOpen(false)
      load()
    } catch (e: any) {
  ...
  const handleUpdate = async (f: AppointmentForm) => {
    ...
    try {
      const payload = mapToPayload(f, packages, {})
      await counsellingApi.updateAppointment(editRow._id, payload)
      setEditRow(null)
      load()
    } catch (e: any) {
  ...
  const handleDelete = async () => {
    ...
    try {
      await counsellingApi.deleteAppointment(deleteRow._id)
      setDeleteRow(null)
      load()
    } catch (e: any) {
  ...
  return (
    <div>
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <h2 className="text-xl font-semibold text-slate-800">Appointments (Counselling)</h2>
        <button
          onClick={() => {
            const f = emptyForm()
            setCreateInitial(f)
                  ...
                  <td className="px-4 py-2 text-right">
                    <div className="flex items-center justify-end gap-1">
                      <button
                        type="button"
                        onClick={() => {
                          nav('/counselling/token-generator', {
                            state: {
                              from: 'appointments',
                              appointmentId: r._id,
                              patient: {
