...
function PayDialog({ open, onClose, row, payToAccounts, onPaid }: { open: boolean; onClose: ()=>void; row: CreditPatientRow | null; payToAccounts: Array<{ code: string; label: string }>; onPaid: (slip: HospitalCreditPaymentSlipData)=>void }){
  ...
  async function submit(){
    ...
    try {
      setBusy(true)
      const res: any = await hospitalApi.payCreditPatient(row.patientId, {
        amount: amt,
...
export default function Hospital_CreditPatients(){
  ...
  async function load(){
    try {
      setLoading(true)
      const res: any = await hospitalApi.listCreditPatients({ q, page, limit })
      setItems(res?.items || [])
  ...
  return (
    <div>
      ...
      {slipOpen && slipData && (
        <Hospital_CreditPaymentSlip open={slipOpen} onClose={()=>setSlipOpen(false)} data={slipData} autoPrint={true} />
      )}
    </div>
  )
}
