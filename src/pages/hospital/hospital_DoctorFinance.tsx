...
export default function Hospital_DoctorFinance() {
  ...
  const [netPaidAll, setNetPaidAll] = useState<number>(0)
  ...
  async function syncBackendEarnings(){
    try {
      ...
      const [earnRes, payoutRes] = await Promise.all([
        financeApi.doctorEarnings(params),
        financeApi.listPayouts({ doctorId: params.doctorId, from: useFrom, to: useTo, limit: 500 }),
      ])

      try {
        const allPayoutsRes: any = await financeApi.listPayouts({ from: useFrom, to: useTo, limit: 5000 })
        const allPayouts: any[] = allPayoutsRes?.payouts || []
        const paidAll = allPayouts.reduce((s, p) => s + Math.abs(Number(p.amount || 0) || 0), 0)
        setNetPaidAll(paidAll)
      } catch { setNetPaidAll(0) }

      const earnings: any[] = (earnRes as any)?.earnings || []
      const payouts: any[] = (payoutRes as any)?.payouts || []
      const mapDoc = (id?: string)=> doctors.find(d=>d.id===id)?.name || 'Doctor'

      const earningsEntries: Entry[] = (Array.isArray(earnings) ? earnings : []).map((r:any)=> ({
        id: `be:${r.id}`,
      ...
      const payoutEntries: Entry[] = (Array.isArray(payouts) ? payouts : []).map((p:any)=> ({
        id: `bp:${p.id}`,
        datetime: `${p.dateIso}T00:00:00`,
        doctorId: p.doctorId,
        doctorName: mapDoc(p.doctorId),
        type: 'Payout',
        description: p.memo,
        doctorAmount: -Math.abs(Number(p.amount||0)),
      }))

      const combined = [...earningsEntries, ...payoutEntries]
  ...
  const summary = useMemo(() => {
    ...
    for (const e of base) {
      ...
      if (t === 'payout') {
        paid += Math.abs(Number(e.doctorAmount || 0) || 0)
      } else {
        earnings += Number(e.doctorAmount || 0) || 0
        if (t === 'opd') opd += Number(e.doctorAmount || 0) || 0
      }
    }

    const due = earnings - paid
    return { opd, earnings, paid, due }
  }, [entries, from, to, doctorId])
  ...
  return (
    <div className="w-full px-4 py-6 sm:px-6 sm:py-8 space-y-4">
      ...
      <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
        ...
        <SummaryCard title="Net Paid" amount={netPaidAll} tone="rose" subtitle="All doctors" />
  ...
  async function deleteEntry(id: string) {
    ...
    if (id.startsWith('be:') || id.startsWith('bp:')){
...
function SummaryCard({ title, amount, tone, subtitle }: { title: string; amount: number; tone: 'emerald'|'sky'|'violet'|'amber'|'rose'; subtitle?: string }) {
  const toneMap: any = {
