  ...
  useEffect(() => {
    ...
    ;(async () => {
      try {
        ...
        if (data.sexualHistory) setSexualHistory((s) => ({ ...s, ...data.sexualHistory }))
      ...
      <div className="mt-3 flex items-center gap-2 text-sm">
        ...
        <button
          ...
          onClick={() => {
            ...
            navigate(`/hospital/lab-reports-entry?tokenId=${encodeURIComponent(patientKey)}`)
          }}
                  ...
                  <div className="mt-3 grid gap-2 sm:grid-cols-2">
                    {(['Right','Left'] as const).map(opt => (
                      <label key={opt} className={checkboxCardLabelCls(sexualHistory.oe.epidCst.side === opt)}>
                        <input type="checkbox" checked={sexualHistory.oe.epidCst.side === opt} onChange={(e)=>setSexualHistory(s=>({ ...s, oe: { ...s.oe, epidCst: { ...s.oe.epidCst, side: e.target.checked ? opt : '' } } }))} className="h-4 w-4 rounded border-slate-300" />
                        {opt}
                      </label>
                    ))}
                  </div>
                  ...
                  <div className="mt-3 grid gap-2 sm:grid-cols-2">
                    {(['Right','Left'] as const).map(opt => (
                      <label key={opt} className={checkboxCardLabelCls(sexualHistory.oe.varicocele.side === opt)}>
                        <input type="checkbox" checked={sexualHistory.oe.varicocele.side === opt} onChange={(e)=>setSexualHistory(s=>({ ...s, oe: { ...s.oe, varicocele: { ...s.oe.varicocele, side: e.target.checked ? opt : '' } } }))} className="h-4 w-4 rounded border-slate-300" />
                        {opt}
                      </label>
                    ))}
                  </div>
