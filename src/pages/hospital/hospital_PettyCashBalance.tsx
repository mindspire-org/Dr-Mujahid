...
export default function Hospital_PettyCashBalance() {
  ...
  useEffect(() => {
    let cancelled = false
      ; (async () => {
        try {
          const mine: any = await hospitalApi.myPettyCash()
  ...
  useEffect(() => {
    let cancelled = false
      ; (async () => {
        ...
        try {
          const r: any = await hospitalApi.listMyPettyCashPullHistory()
  ...
  useEffect(() => {
    try { localStorage.setItem('hospital.petty.senderAccountCode', String(senderAccountCode || '')) } catch { }
  }, [senderAccountCode])

  useEffect(() => {
    setPettyFrom(dateIso)
    setPettyTo(dateIso)
  }, [dateIso])
  ...
  const entries = useMemo(() => {
    const acct = String(pettyFilterCode || '').trim().toUpperCase()
    if (acct) {
      // Prefer the authoritative ledger (FinanceJournal) so OPD token receipts are visible.
    ...
    // Fallback: opening + pull history (legacy view)
    ...
    const pulls = pullHistory.map(h => {
      ...
      return { id: h.id, txId: h.txId, dateIso: h.dateIso, createdAt: h.createdAt, account: pettyCode, amount: net, memo: h.memo, refId: h.refId, transferFrom: fromCode || undefined, transferTo: toCode || undefined }
    }).filter(e => {
      if (pettyFrom && String(e.dateIso || '') < pettyFrom) return false
      if (pettyTo && String(e.dateIso || '') > pettyTo) return false
      return true
    })
    return [...openingPetty, ...pulls]
  }, [openingEntries, pullHistory, pettyAccounts, pettyFilterCode, pettyFrom, pettyTo, pettyCode, ledgerRows])

  const { totalDebit, totalCredit, pettyBalance } = useMemo(() => {
    let debit = 0
    let credit = 0
    ...
    return { totalDebit: debit, totalCredit: credit, pettyBalance: debit - credit }
  }, [entries])
    ...
    <div className="w-full px-3 py-4 space-y-4">
      ...
      <div className="grid gap-4 sm:grid-cols-3">
        <div className="rounded-xl border border-slate-200 bg-white p-4">
          ...
          <div className="mt-1 text-2xl font-semibold">Rs. {compactAmount(Number(pettyBalance || 0))}</div>
        </div>
        <div className="rounded-xl border border-slate-200 bg-white p-4">
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          ...
          <div className="sm:col-span-2">
            ...
            <select value={senderAccountCode} onChange={e => setSenderAccountCode(e.target.value)} className="w-full rounded-md border border-slate-300 px-3 py-2 text-sm">
              ...
              {pettyAccounts
                .filter(a => String(a.code || '').trim().toUpperCase() !== String(pettyCode || '').trim().toUpperCase())
                .map(a => (
          ...
          <div className="relative z-50 w-full sm:max-w-lg bg-white rounded-t-2xl sm:rounded-2xl shadow-lg p-4 m-0 sm:m-4">
            ...
            <div className="mt-3 grid grid-cols-1 sm:grid-cols-2 gap-3 text-sm">
              ...
              <div><div className="text-slate-500">Account Name(Sender)</div><div>{(() => {
                const code = String(viewTxn.senderAccount || '').trim().toUpperCase()
                if (!code) return '-'
