...
export default function TherapyLab_Header({ 
  title = 'Therapy & Lab Reports Entry',
  onToggleSidebar,
  collapsed,
  onToggleTheme,
  theme
  ...
  useEffect(() => {
    try {
      const raw = localStorage.getItem('therapyLab.session') || localStorage.getItem('hospital.session') || localStorage.getItem('user') || '{}'
    ...
    >
      <div className="flex h-full items-center">
        ...
        <div className="flex h-full flex-1 items-center gap-0 px-3 sm:px-5">
          ...
          <Link to="/therapy-lab/token-generator" className="flex min-w-0 items-center gap-2">
            ...
            <div className="min-w-0 font-semibold text-white text-sm sm:text-base truncate">
              {String(brand.name || '').trim() || DEFAULT_HOSPITAL_NAME}
            </div>
            <span className="ml-2 hidden lg:inline-flex rounded-full bg-sky-100 px-2 py-0.5 text-[10px] font-medium text-sky-700 whitespace-nowrap">{title}</span>
          </Link>
          ...
          <div className="ml-auto flex items-center gap-3 text-sm">
            ...
            <div className="hidden items-center gap-2 text-white/85 sm:flex">
              <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="h-4 w-4">
                <path d="M6.75 3A2.75 2.75 0 0 0 4 5.75v12.5A2.75 2.75 0 0 0 6.75 21h10.5A2.75 2.75 0 0 0 20 18.25V5.75A2.75 2.75 0 0 0 17.25 3H6.75Zm0 1.5h10.5c.69 0 1.25.56 1.25 1.25v12.5c0 .69-.56 1.25-1.25 1.25H6.75c-.69 0-1.25-.56-1.25-1.25V5.75c0-.69.56-1.25 1.25-1.25Z" />
              </svg>
            ...
            <button onClick={handleToggleTheme} className="hidden rounded-md border border-white/15 px-3 py-1.5 text-white hover:bg-white/10 sm:flex items-center gap-2">
              <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="h-4 w-4">
                <path d="M7.5 3h9A2.5 2.5 0 0 1 19 5.5v13A2.5 2.5 0 0 1 16.5 21h-9A2.5 2.5 0 0 1 5 18.5v-13A2.5 2.5 0 0 1 7.5 3Zm0 2A.5.5 0 0 0 7 5.5v13a.5.5 0 0 0 .5.5h9a.5.5 0 0 0 .5-.5v-13a.5.5 0 0 0-.5-.5h-9Z" />
              </svg>
