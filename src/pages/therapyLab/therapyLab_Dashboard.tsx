...
export default function TherapyLab_Dashboard() {
  ...
  useEffect(() => {
    ...
    ;(async () => {
      try {
        const [summary, packagesPage, weeklyVisits] = await Promise.all([
          therapyApi.getDashboardSummary({ from: effFrom, to: effTo }) as any,
          therapyApi.listPackages({ limit: 1 }) as any,
          therapyApi.listVisits({ from: from8w, to: todayStr, limit: 1000 }) as any,
        ])
        if (!mounted) return
        setTokensToday(Number(summary?.visits || 0))
        setTotalPackages(Number(summary?.totalPackages || packagesPage?.total || 0))
        ...
        const weeklyArr = Array.isArray(weeklyVisits?.items) ? weeklyVisits.items : (Array.isArray(weeklyVisits?.visits) ? weeklyVisits.visits : [])
        const sorted = [...weeklyArr].sort((a: any, b: any) => new Date(b?.createdAtIso || b?.createdAt || 0).getTime() - new Date(a?.createdAtIso || a?.createdAt || 0).getTime())
        ...
        for (const o of weeklyArr) {
          const dt = o?.createdAtIso ? new Date(o.createdAtIso) : (o?.createdAt ? new Date(o.createdAt) : null)
          if (!dt) continue
  ...
  return (
    <div className="space-y-4">
      <h2 className="text-xl font-semibold text-slate-800 ">Therapy Dashboard</h2>
          ...
          <ul className="space-y-2 text-sm">
            {recentSales.map((o: any, idx: number) => (
              <li key={String(o?._id || idx)} className="flex items-center justify-between rounded-md border border-slate-100 px-3 py-2 ">
                <div>
                  <div className="font-medium text-slate-800">{formatDate(o?.createdAtIso || o?.createdAt)}</div>
            ...
            {recentSales.length === 0 && <li className="text-slate-500">No recent tokens</li>}
          </ul>
        </div>
      </div>
    </div>
