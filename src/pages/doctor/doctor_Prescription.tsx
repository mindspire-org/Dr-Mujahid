...
export default function Doctor_Prescription() {
  ...
  const [activeTab, setActiveTab] = useState<'details'|'vitals'|'labs'|'diagnostics'|'therapy'>('details')
  useEffect(() => {
    try {
  ...
  const goTab = (tab: 'details'|'vitals'|'labs'|'diagnostics'|'therapy') => {
    if (activeTab === 'vitals') {
      try {
    ...
    <div className="w-full px-2 sm:px-4">
      ...
      <form onSubmit={save} className="mt-4 space-y-4 rounded-xl border border-slate-200 bg-white p-5">
        ...
        {activeTab==='therapy' && (
          <div className="grid gap-4 sm:grid-cols-2">
            <div>
              <label className="mb-1 block text-sm text-slate-700">Therapy Orders (comma or one per line)</label>
              <SuggestField mode="lab-tests" rows={3} value={(form as any).therapyTestsText} onChange={v=>setForm(f=>({ ...(f as any), therapyTestsText: v }))} suggestions={sugLabTests} placeholder="Physiotherapy, Exercise" />
            </div>
            <div>
              <label className="mb-1 block text-sm text-slate-700">Therapy Notes</label>
              <SuggestField rows={2} value={(form as any).therapyNotes} onChange={v=>setForm(f=>({ ...(f as any), therapyNotes: v }))} suggestions={sugLabNotes} />
            </div>
          </div>
        )}
