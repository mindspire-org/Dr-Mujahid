...
async function normalizeDoctorDepartments(docData: any){
  ...
  const merged = primary ? Array.from(new Set([...ids, primary])) : ids
  docData.departmentIds = merged
  if (!docData.primaryDepartmentId) delete docData.primaryDepartmentId

  const toCheck = Array.from(new Set([...(docData.departmentIds || []), ...(primary ? [primary] : [])]))
  if (!toCheck.length) return
  const existing = await HospitalDepartment.find({ _id: { $in: toCheck } }).select('_id').lean()
  const ok = new Set((existing || []).map((d: any)=> String(d._id)))
  const invalid = toCheck.filter(id => !ok.has(String(id)))
  if (invalid.length) {
    const err: any = new Error('Invalid departmentId(s)')
    err.statusCode = 400
    ;(err as any).details = invalid
    throw err
  }
}
...
export async function createDoctor(req: Request, res: Response){
  ...
  try {
    await normalizeDoctorDepartments(docData)
  } catch (e: any) {
    const msg = e?.message || 'Invalid departmentId(s)'
...
export async function updateDoctor(req: Request, res: Response){
  ...
  try {
    await normalizeDoctorDepartments(patch)
  } catch (e: any) {
    const msg = e?.message || 'Invalid departmentId(s)'
    return res.status(400).json({ error: msg, details: (e as any)?.details })
  }
