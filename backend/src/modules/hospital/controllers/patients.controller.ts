...
import { TherapyVisit } from '../../therapy/models/TherapyVisit'
import { CounsellingOrder } from '../../counselling/models/CounsellingOrder'
import { CounsellingPackage } from '../../counselling/models/CounsellingPackage'
...
export async function profile(req: Request, res: Response){
  try {
    ...
    const therapyRows = await TherapyVisit.find({ patientId }).sort({ createdAt: -1 }).limit(50).lean()
    const therapy = therapyRows.map((v: any) => ({
    ...
    const counsellingRows = await CounsellingOrder.find({ patientId }).sort({ createdAt: -1 }).limit(50).populate('packages').lean()
    const counselling = counsellingRows.map((o: any) => ({
      id: String(o._id || o.id),
      createdAt: o.createdAt,
      status: o.sessionStatus || 'Pending',
      packageName: o.packages?.map((p: any) => p.packageName || p.name).join(', ') || '-',
      tests: o.packages?.map((p: any) => ({ name: p.packageName || p.name, price: p.price })) || [],
    ...
    res.json({ patient, pres, lab, diag, therapy, counselling })
  } catch (e: any) {
...
export async function exportPatients(req: Request, res: Response) {
  try {
    ...
    // Fetch patients and sort by MRN (treating it as numeric if possible)
    let pats: any[] = await LabPatient.find(filter).lean();
    
    // Sort patients by MRN numerically
    pats.sort((a, b) => {
      const mrnA = parseInt(String(a.mrn || '').replace(/\D/g, '')) || 0;
      const mrnB = parseInt(String(b.mrn || '').replace(/\D/g, '')) || 0;
      return mrnA - mrnB;
    });
    ...
    const headers = ['MR Number', 'Patient Name', 'Age', 'Gender', 'Phone', 'CNIC', 'Address'];
    ...
    if (pats.length > 0) {
      pats.forEach(p => {
        const row = [
          ...
          `="${String(p.phoneNormalized || p.phone || '').replace(/"/g, '""')}"`,
          `="${String(p.cnicNormalized || p.cnic || p.cnic_no || p.identity_no || p.nic || '').replace(/"/g, '""')}"`,
