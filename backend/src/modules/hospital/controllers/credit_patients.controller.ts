...
export async function listCreditPatients(req: Request, res: Response){
  try {
    ...
    const [accts, unpaidPatientIds] = await Promise.all([
      HospitalAccount.find(acctFilter).sort({ updatedAt: -1 }).limit(2000).lean(),
      HospitalToken.distinct('patientId', { paymentStatus: 'unpaid' }),
    ])
    ...
    const recent = await HospitalToken.find({ patientId: { $in: allIds } })
      .sort({ createdAt: -1 })
      .select(
        'patientId createdAt tokenNo fee amount discount paymentStatus receptionistName paymentMethod accountNumberIban receivedToAccountCode ' +
...
export async function payCreditPatient(req: Request, res: Response){
  try {
    ...
    const acct: any = await HospitalAccount.findOneAndUpdate(
      { patientId },
      {
    ...
    // Finance: debit settlement (received account), credit AR
    try {
      await FinanceJournal.create({
        dateIso: new Date().toISOString().slice(0, 10),
        createdBy,
        refType: 'hospital_credit_patient_payment',
        refId: patientId,
        memo: `Credit Patient Payment — ${String(p?.fullName || '').trim()}`.trim(),
        lines: [
          { account: receivedToAccountCode, debit: amount, tags: { patientName: String(p?.fullName || '').trim(), mrn: String(p?.mrn || '').trim(), patientId } },
          { account: 'AR', credit: amount, tags: { patientName: String(p?.fullName || '').trim(), mrn: String(p?.mrn || '').trim(), patientId } },
        ],
      })
    } catch {}
