...
import { DiagnosticAppointment } from '../models/Appointment'
...
export async function list(req: Request, res: Response){
  try {
    const parsed = diagnosticAppointmentQuerySchema.safeParse(req.query)
    ...
    const [items, total] = await Promise.all([
      DiagnosticAppointment.find(filter)
        .sort({ appointmentDate: -1, createdAt: -1 })
        .skip(skip)
        .limit(lim)
        .lean(),
      DiagnosticAppointment.countDocuments(filter),
    ])
...
export async function getById(req: Request, res: Response){
  try {
    ...
    const doc = await DiagnosticAppointment.findById(id).lean()
...
export async function update(req: Request, res: Response){
  try {
    const id = String(req.params.id || '')
    const data = diagnosticAppointmentUpdateSchema.parse(req.body)
    ...
    const doc = await DiagnosticAppointment.findByIdAndUpdate(id, { $set: patch }, { new: true })
    if (!doc) return res.status(404).json({ error: 'Appointment not found' })
    res.json({ appointment: doc })
  } catch (e){
    return handleError(res, e)
...
export async function remove(req: Request, res: Response){
  try {
    const id = String(req.params.id || '')
    const doc = await DiagnosticAppointment.findByIdAndDelete(id)
    if (!doc) return res.status(404).json({ error: 'Appointment not found' })
    res.json({ ok: true })
  } catch (e){
    return handleError(res, e)
