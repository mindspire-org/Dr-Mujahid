...
export const listCreditPatients = async (req: Request, res: Response) => {
  try {
    ...
    const [accts, unpaidPatientIds] = await Promise.all([
      CounsellingAccount.find(acctFilter).sort({ updatedAt: -1 }).limit(2000).lean(),
      CounsellingOrder.distinct('patientId', { paymentStatus: 'unpaid' }),
    ])
    ...
    // Fetch recent orders for these patients
    const recent = await CounsellingOrder.find({ patientId: { $in: allIds } })
      .sort({ createdAt: -1 })
      .select(
...
export const payCreditPatient = async (req: Request, res: Response) => {
  try {
    ...
    const acct: any = await CounsellingAccount.findOneAndUpdate(
      { patientId },
      {
    ...
    // Finance: debit settlement (received account), credit AR_COUNSELLING
    try {
      await FinanceJournal.create({
        dateIso: new Date().toISOString().slice(0, 10),
        createdBy,
        refType: 'Couns_Credit_Rev',
        refId: patientId,
        memo: `Counselling Credit Patient Payment — ${String(p?.fullName || '').trim()}`.trim(),
        lines: [
          { account: receivedToAccountCode, debit: amount, tags: { patientName: String(p?.fullName || '').trim(), mrn: String(p?.mrn || '').trim(), patientId } },
          { account: 'AR_COUNSELLING', credit: amount, tags: { patientName: String(p?.fullName || '').trim(), mrn: String(p?.mrn || '').trim(), patientId } },
        ],
      })
    } catch { }
