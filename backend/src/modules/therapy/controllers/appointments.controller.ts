...
import { TherapyAppointment } from '../models/TherapyAppointment'
import { therapyAppointmentCreateSchema, therapyAppointmentQuerySchema, therapyAppointmentUpdateSchema } from '../validators/appointment'
...
function handleError(res: Response, e: any){
  ...
  console.error('[TherapyAppointments]', e)
...
export async function list(req: Request, res: Response){
  try {
    const parsed = therapyAppointmentQuerySchema.safeParse(req.query)
    ...
    const [items, total] = await Promise.all([
      TherapyAppointment.find(filter)
        .sort({ appointmentDate: -1, createdAt: -1 })
        .skip(skip)
        .limit(lim)
        .lean(),
      TherapyAppointment.countDocuments(filter),
    ])
...
export async function getById(req: Request, res: Response){
  try {
    ...
    const doc = await TherapyAppointment.findById(id).lean()
...
export async function create(req: Request, res: Response){
  try {
    const data = therapyAppointmentCreateSchema.parse(req.body)
    ...
    const doc = await TherapyAppointment.create({
      ...data,
...
export async function update(req: Request, res: Response){
  try {
    const id = String(req.params.id || '')
    const data = therapyAppointmentUpdateSchema.parse(req.body)
    ...
    const doc = await TherapyAppointment.findByIdAndUpdate(id, { $set: patch }, { new: true })
    if (!doc) return res.status(404).json({ error: 'Appointment not found' })
    res.json({ appointment: doc })
  } catch (e){
    return handleError(res, e)
...
export async function remove(req: Request, res: Response){
  try {
    const id = String(req.params.id || '')
    const doc = await TherapyAppointment.findByIdAndDelete(id)
    if (!doc) return res.status(404).json({ error: 'Appointment not found' })
    res.json({ ok: true })
  } catch (e){
    return handleError(res, e)
